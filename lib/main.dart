...
class _ControllerScreenState extends State<ControllerScreen> {
  ...
  String _frame = "Base";
  String _motionType = "JOG"; // JOG or MOVE

  // Navigation State
  String _currentView = "MAIN"; // MAIN, CARTESIAN, JOINTS
  ...
  void _connect() {
    ...
    try {
      ...
      _channel!.stream.listen(
        ...
        onDone: () {
          // If connection drops immediately while waiting, it means the server rejected us
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: _currentView == "MAIN"
            ? AppBar(
                title: const Text(
                  "Texsonics Controller", 
                  style: TextStyle(fontWeight: FontWeight.bold, letterSpacing: 0.5, fontSize: 17)
                ),
                actions: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: _wsStatusColor.withOpacity(0.2),
                      borderRadius: BorderRadius.circular(20),
                      border: Border.all(color: _wsStatusColor),
                    ),
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Expanded(child: _buildColorButton("CONNECT", AppColors.accentBlue, _connect, padding: 0)),
                    const SizedBox(width: 8),
                    Expanded(child: _buildColorButton("DISCONNECT", AppColors.accentRed, _disconnect, padding: 0)),
                  ],
                ),
              ],
            ),
          ...
          Expanded(
            child: ListView(
              ...
              children: [
                ...
                Row(
                  children: [
                    Expanded(child: _buildColorButton("AUTO", AppColors.accentBlue, () => _sendCommand('SET_AUTO'), padding: 0)),
                    const SizedBox(width: 10),
                    Expanded(child: _buildColorButton("MAN", AppColors.accentBlue, () => _sendCommand('SET_MANUAL'), padding: 0)),
                    const SizedBox(width: 10),
                    Expanded(child: _buildColorButton("REM", AppColors.accentBlue, () => _sendCommand('SET_REMOTE'), padding: 0)),
                  ],
                ),
                const SizedBox(height: 25),
                const Text("CONFIGURATION", style: TextStyle(color: AppColors.accentBlue, fontWeight: FontWeight.bold, fontSize: 12)),
                const SizedBox(height: 10),
                _buildDrawerDropdownRow("Lin Inc (mm)", _selectedMmInc, _mmOptions, (val) {
                ...
                _buildDrawerDropdownRow("Ang Inc (deg)", _selectedDegInc, _degOptions, (val) {
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                GestureDetector(
                  onTap: () {
                    if (_errorMsg != "No error") _showErrorPopup(_errorMsg);
                  },
                  child: _buildStatusItem("ERR", _errorMsg == "No error" ? "NONE" : "ERR", _errorMsg == "No error" ? AppColors.accentGreen : AppColors.accentRed),
                ),
              ],
            ),
          ),
          ...
          GestureDetector(
            ...
            child: _buildNavCard("CARTESIAN PAD", Icons.screen_rotation, "Opens in Landscape"),
          ),
          const SizedBox(height: 15),
          GestureDetector(
            onTap: _openJoints,
            child: _buildNavCard("JOINTS PAD", Icons.precision_manufacturing, "Opens in Portrait"),
          ),
        ],
      ),
    );
        ...
        children: [
          ...
          Center(
            child: Row(
              ...
              children: [
                _buildDPadCluster("Y+", "Y-", "X-", "X+", "Z+", "Z-", "LINEAR"),
                Container(width: 2, height: 200, color: AppColors.border),
                _buildDPadCluster("Rx+", "Rx-", "Ry+", "Ry-", "Rz+", "Rz-", "ANGULAR"),
              ],
            ),
          ),
          Positioned(
            top: 20, right: 30,
            child: Text("ACTIVE: $_motionType", style: TextStyle(color: _motionType == "JOG" ? AppColors.accentGreen : AppColors.accentBlue, fontWeight: FontWeight.bold, fontSize: 18, letterSpacing: 2)),
          )
        ],
      ),
    );
  ...
  Widget _buildDPadCluster(String up, String down, String left, String right, String zUp, String zDown, String title) {
    return Column(
      ...
      children: [
        Text(title, style: const TextStyle(color: Colors.grey, fontWeight: FontWeight.bold, letterSpacing: 2)),
        const SizedBox(height: 20),
        Row(
          mainAxisSize: MainAxisSize.min,
          ...
          children: [
            Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                _buildJogButton(up, width: 70, height: 60),
                Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    _buildJogButton(left, width: 70, height: 60),
                    const SizedBox(width: 65), // Empty Center
                    _buildJogButton(right, width: 70, height: 60),
                  ],
                ),
                _buildJogButton(down, width: 70, height: 60),
              ],
            ),
            ...
            Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                _buildJogButton(zUp, width: 70, height: 80),
                const SizedBox(height: 30),
                _buildJogButton(zDown, width: 70, height: 80),
              ],
            )
          ],
        ),
  ...
  Widget _buildGenericButton(String text, VoidCallback onPressed) {
    return ElevatedButton(
      style: ElevatedButton.styleFrom(
        backgroundColor: AppColors.btnBg, foregroundColor: Colors.white,
